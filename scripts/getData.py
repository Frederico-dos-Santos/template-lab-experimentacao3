...
def run_query(query, variables=None):
    request = requests.post(API_URL, json={"query": query, "variables": variables}, headers=HEADERS)
    if request.status_code == 200:
        return request.json()
    else:
        raise Exception("Query failed to run by returning code of {}. {}".format(request.status_code, query))
    
def get_repo_data(num_repos: int, per_page: int) -> list:
    ...
    try:
        ...
        end_time = time.time()
        print(f"{len(results)} repositórios encontrados")
        ...
        for repo in repos:
            ...
            for edge in data["data"]["repository"]["pullRequests"]["edges"]:
                ...
                if node["mergedAt"]:
                    ...
                    interval_dates = max(data_merge_dt, data_fechamento_dt) - data_criacao_dt
                else:
                    data_merge_dt = None
