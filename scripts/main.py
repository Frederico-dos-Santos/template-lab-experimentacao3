...
def main():
    ...
    repo_results = read_csv("raw_repo_data.csv", columns=[
                            'Repositório', 'Estrelas', 'Pull Requests'])
    ...
    mean_metrics = ['Intervalo Criação e Última Atividade',
                    'Participantes PR', 'Caracteres Corpo PR']
    total_metrics = ['Revisões', 'Comentários PR',
                     'Arquivos Alterados', 'Linhas Adicionadas', 'Linhas Excluídas']
    data_summarized = summarized_data(
        data_results, mean_columns=mean_metrics, total_columns=total_metrics)
    ...
    df_changes_req = data_results[data_results["Status Revisão"]
                                  == "CHANGES_REQUESTED"]
    df_review_req = data_results[data_results["Status Revisão"]
                                 == "REVIEW_REQUIRED"]

    metrics_sum = ["Arquivos Alterados", "Linhas Adicionadas",
                   "Linhas Excluídas", "Caracteres Corpo PR"]
    novo_df_approved = df_approved.groupby(
        "Repositório")[metrics_sum].sum().reset_index()
    novo_df_changes_req = df_changes_req.groupby(
        "Repositório")[metrics_sum].sum().reset_index()
    novo_df_review_req = df_review_req.groupby(
        "Repositório")[metrics_sum].sum().reset_index()

    boxplot(
        data=[novo_df_approved['Arquivos Alterados'],
              novo_df_changes_req['Arquivos Alterados'], novo_df_review_req['Arquivos Alterados']],
    ...
    boxplot(
        data=[filtered_df_approved['Linhas Adicionadas'],
              novo_df_changes_req['Linhas Adicionadas'], filtered_df_review_req['Linhas Adicionadas']],
        columns=["APPROVED", "CHANGES_REQUESTED", "REVIEW_REQUIRED"],
        x_label="Feedback final das Revisões",
    ...
    boxplot(
        data=[filtered_df_approved['Linhas Excluídas'],
              novo_df_changes_req['Linhas Excluídas'], filtered_df_review_req['Linhas Excluídas']],
        columns=["APPROVED", "CHANGES_REQUESTED", "REVIEW_REQUIRED"],
        x_label="Feedback final das Revisões",
    ...
    metrics_mean = ["Intervalo Criação e Última Atividade",
                    "Participantes PR", "Comentários PR"]
    df_approved_mean = df_approved.groupby(
        "Repositório")[metrics_mean].mean().reset_index()
    df_changes_req_mean = df_changes_req.groupby(
        "Repositório")[metrics_mean].mean().reset_index()
    df_review_req_mean = df_review_req.groupby(
        "Repositório")[metrics_mean].mean().reset_index()

    boxplot(
        data=[df_approved_mean['Intervalo Criação e Última Atividade'],
              df_changes_req_mean['Intervalo Criação e Última Atividade'], df_review_req_mean['Intervalo Criação e Última Atividade']],
        columns=["APPROVED", "CHANGES_REQUESTED", "REVIEW_REQUIRED"],
        x_label="Feedback final das Revisões",
    ...
    boxplot(
        data=[df_approved_mean['Intervalo Criação e Última Atividade'],
              df_review_req_mean['Intervalo Criação e Última Atividade']],
    ...
    boxplot(
        data=[filtered_df_approved['Caracteres Corpo PR'],
              novo_df_changes_req['Caracteres Corpo PR'], filtered_df_review_req['Caracteres Corpo PR']],
        columns=["APPROVED", "CHANGES_REQUESTED", "REVIEW_REQUIRED"],
        x_label="Feedback final das Revisões",
    ...
    boxplot(
        data=[df_approved_mean['Participantes PR'],
              df_changes_req_mean['Participantes PR'], df_review_req_mean['Participantes PR']],
        columns=["APPROVED", "CHANGES_REQUESTED", "REVIEW_REQUIRED"],
        x_label="Feedback final das Revisões",
    ...
    boxplot(
        data=[df_approved_mean['Comentários PR'],
              df_changes_req_mean['Comentários PR'], df_review_req_mean['Comentários PR']],
        columns=["APPROVED", "CHANGES_REQUESTED", "REVIEW_REQUIRED"],
        x_label="Feedback final das Revisões",
    ...
    test = data_summarized[data_summarized["Intervalo Criação e Última Atividade"] <= 1000]

    dispertion(
        test,
        color="purple",
        columns={"x": "Revisões", "y": "Intervalo Criação e Última Atividade"},
        labels={"x": "Total de Revisões",
                "y": "Intervalo Criação e Última Atividade"},
